"""
In this script, we're preparing the density datasets for loading into an Observable
...
# First, we'll find all of the datasets files
density_folder = "datasets/density"
...
for density_file, spp_name in zip(density_img_files, species_name):
    ...
    # There are 12 months of density datasets available
    if count_of_species.get(spp_name)==12:
        month_number = density_file.split('month')[1].split('.img')[0]
        out_file = f"datasets/density_geotiffs/{spp_name}.month{month_number}.tif"
    ...
    # There's only an annual average
        ...
        for month_num_val in np.arange(1,13):
            out_file = f"datasets/density_geotiffs/{spp_name}.month{month_num_val:02}.tif"
            img_to_geotiff(density_file, out_file)
